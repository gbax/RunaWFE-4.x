...
public class AjaxActorsList extends JsonAjaxCommand {
	...
	private BatchPresentation getPresentation(String target, Integer page, Integer perPage, String hint) {
        ...
        if (hint.length() > 0) {
            batchPresentation.getFilteredFields().put(filterIndex, new StringFilterCriteria(StringFilterCriteria.ANY_SYMBOLS + hint + StringFilterCriteria.ANY_SYMBOLS));
        }
	...
	protected JSONAware execute(User user, HttpServletRequest request)
		...
		for (Executor executor : getExecutors(presentation)) {
			if(isExcluded(executor)) continue;
