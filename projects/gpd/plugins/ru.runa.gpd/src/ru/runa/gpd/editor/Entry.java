...
public abstract class Entry implements GEFConstants {
    ...
    public Dimension getDefaultSize() {
        if (!fixedSize) {
            ...
            String key = LanguageElementPreferenceNode.getId(nodeTypeDefinition, Language.JPDL) + '.' + PrefConstants.P_LANGUAGE_NODE_WIDTH;
            if (store.contains(key)) {
                defaultSize.setWidth(GRID_SIZE * store.getInt(key));
            }
            key = LanguageElementPreferenceNode.getId(nodeTypeDefinition, Language.JPDL) + '.' + PrefConstants.P_LANGUAGE_NODE_HEIGHT;
            if (store.contains(key)) {
